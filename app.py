...
# =========================
st.set_page_config(
    page_title="物理前沿知识融入初中物理教学案例展示平台",
    page_icon="🔬",
    layout="wide",
...
.card {
    ...
    box-shadow: 0 6px 18px rgba(32, 71, 120, 0.06);
    height: 100%;
}
.metric-card {
...
def pil_placeholder(title: str, subtitle: str, accent=(33, 78, 122)) -> Image.Image:
    ...
    draw.text((85, 220), "IMAGE PLACEHOLDER", fill=accent, font=font_big)
    draw.text((85, 420), subtitle, fill=(70, 80, 95), font=font_sub)
    draw.text((85, 470), "可在 assets 文件夹放入对应图片，或在应用中上传图片替换", fill=(90, 100, 115), font=font_sub)
    return img
...
def make_lidar_figure(distance_m: float = 30.0, angle_deg: float = 0.0):
    ...
    ax.add_patch(sensor)
    ax.add_patch(target)
    ax.text(1.4, 3.0, "激光雷达", ha="center", va="center", fontsize=12)
    ax.text(9.7, target_y, "目标", ha="center", va="center", fontsize=12)
    ...
    ax.text(5.5, 4.5, "发射光束", fontsize=11)
    ax.text(5.4, 1.8, "反射返回", fontsize=11)
    ax.text(6.0, 5.3, r"测距公式：$d=\frac{ct}{2}$", fontsize=14, ha="center")
    ax.set_title("激光雷达测距原理示意图")
    fig.tight_layout()
    return fig
...
def make_battery_figure(n_series: int = 4, n_parallel: int = 2):
    ...
    ax.text((left_bus_x + right_bus_x) / 2, start_y + n_parallel * 1.9 + 0.6, "新能源汽车电池串并联示意图", fontsize=14, ha="center")
    fig.tight_layout()
    return fig
...
def make_mapping_figure():
    ...
    ax.text(2.2, 9.2, "前沿案例", fontsize=13, fontweight="bold")
    ax.text(7.2, 9.2, "对应物理知识", fontsize=13, fontweight="bold")
    ax.text(12.2, 9.2, "教学价值", fontsize=13, fontweight="bold")
    ...
    ax.set_title("前沿知识融入初中物理教学的案例对应关系图", fontsize=14)
    fig.tight_layout()
    return fig
...
def make_interest_comparison(df: pd.DataFrame, case_col: str, pre_col: str, post_col: str):
    ...
    ax.bar(x - width / 2, grouped[pre_col], width, label="前测")
    ax.bar(x + width / 2, grouped[post_col], width, label="后测")
    ax.set_xticks(x)
    ax.set_xticklabels(grouped[case_col])
    ax.set_ylabel("平均得分")
    ax.set_title("案例教学前后兴趣变化对比")
    ax.legend()
...
st.sidebar.caption("建议将图片放在以下位置。")
st.sidebar.code(
    "assets/\n  submarine_structure.png\n  lidar_scan.jpg\n  ev_platform.jpg\n\n或直接与 app.py 放在同一目录",
    language="bash",
)

with st.sidebar.expander("本次运行可上传图片"):
    st.session_state["upload_submarine"] = st.file_uploader("上传深海探测图片", type=["png", "jpg", "jpeg"], key="u1")
    st.session_state["upload_lidar"] = st.file_uploader("上传激光雷达图片", type=["png", "jpg", "jpeg"], key="u2")
...
if page == "首页总览":
    header_block(
        ...
        "围绕深海探测、激光雷达、新能源汽车三个案例，构建用于论文展示与课堂说明的可视化界面。",
    )
    ...
    with c4:
        metric_card("交互模块", "4类", "参数调节、图片展示、关系图、数据分析")
    ...
    with col1:
        card_html(
            "总体说明",
            "本平台围绕“物理前沿知识在初中物理教学中的融合”这一主题，采用案例展示方式，将深海探测、激光雷达、新能源汽车三个前沿情境，与初中物理中的浮力、光现象和电学知识建立对应关系，可用于论文展示、课堂汇报和答辩说明。",
        )
    with col2:
        st.image(
            get_case_image(
    ...
    with col1:
        card_html(
            "案例一：浮力与深海探测",
            "以深海潜航器和海洋探测器为情境，把浮力、阿基米德原理、浮沉条件与液体压强联系起来，帮助学生理解浮力知识在深海工程中的实际作用。",
        )
    with col2:
        card_html(
            "案例二：光现象与激光雷达",
            "通过自动驾驶中的激光雷达，引导学生理解光的传播、反射与测距思想，认识光学知识在现代智能交通中的技术价值。",
        )
    with col3:
        card_html(
            "案例三：电学与新能源汽车",
            "围绕电池、电机和充电系统，连接电流、电压、电阻、串并联和电能转化知识，让学生感受电学与绿色能源技术的现实联系。",
        )

    st.markdown(
        """
        <div class="note-box">
        <b>使用说明：</b> 首页适合展示研究框架；三大案例页可分别截屏作为论文附图；数据分析页适合展示问卷或课堂前后测结果；图片资料库页便于整理与切换素材。
        </div>
        """,
        unsafe_allow_html=True,
    )
...
# =========================
    ...
    with right:
        ...
        st.write(
            "在讲浮力的时候，可以引入深海潜航器和海洋探测器。设备在海中运行时，不仅要解决下沉和上浮的问题，还要适应深海环境中的巨大压强。通过这个案例，学生能够明白浮力知识并不只用于判断物体是否漂浮，还能服务于真实工程技术。"
        )
        with st.expander("可直接写入论文的表述"):
            st.write(
                "深海探测技术为浮力教学提供了真实问题情境。通过分析潜航器在深海中下潜、悬停和上浮的条件，学生能够将浮力、阿基米德原理、浮沉条件和液体压强等知识联系起来，从而增强对物理知识实际应用价值的理解。"
            )
    ...
    with c1:
        card_html(
            "课堂可提问题",
            "1. 为什么深海潜航器不能只考虑会不会浮？<br>2. 深度越大，外壳为什么要更坚固？<br>3. 如果要让探测器重新上浮，可以改变哪些条件？",
        )
    with c2:
        card_html(
            "素养提升点",
            "帮助学生理解物理规律在复杂工程环境中的综合应用，形成从“单一知识点”走向“系统思考”的学习视角。",
        )
...
# =========================
    ...
    with right:
        ...
        st.write(
            "激光雷达的基本原理是发射光信号，再接收反射回来的光，通过光的往返时间判断前方物体的位置和距离。这个案例能把课本中的光学知识转化为实际测量方法，让学生理解光不仅能“看见”物体，还能“测量”物体。"
        )
    ...
    with c1:
        card_html(
            "课堂可提问题",
            "1. 光信号为什么能用于测距？<br>2. 为什么激光雷达不仅要发光，还要接收反射光？<br>3. 如果前方环境更复杂，设备为什么要连续扫描多个方向？",
        )
    with c2:
        card_html(
            "论文可写价值",
            "该案例将光的传播与反射从静态现象转化为信息获取手段，有利于学生理解现代传感技术与基础光学知识之间的内在联系。",
        )
...
# =========================
    ...
    with right:
        ...
        st.write(
            "新能源汽车中涉及电池、电机和充电系统，与初中电学中的电流、电压、电阻和电路连接知识密切相关。通过这个案例，学生可以理解电池串并联的实际意义，认识电能如何转化为机械能，并增强安全用电意识。"
        )
        with st.expander("适合写入论文的简洁表述"):
            st.write(
                "新能源汽车案例把抽象的电学知识放入生活化、时代化的技术情境中，能够帮助学生理解电路连接、电能转化和安全用电等内容的现实意义。"
            )
    ...
    with c1:
        card_html(
            "课堂可提问题",
            "1. 为什么汽车电池不是一节，而是很多节组合？<br>2. 想提高总电压，应该怎么接？<br>3. 为什么充电过程中必须强调安全？",
        )
    with c2:
        card_html(
            "教学价值",
            "该案例与学生日常生活联系紧密，能够增强电学学习的现实感和时代感，同时帮助学生建立绿色能源和技术应用的基本认识。",
        )
...
# =========================
    ...
    header_block("前沿知识融入初中物理教学的案例对应关系", "通过关系图呈现三个案例与物理知识、教学价值之间的对应结构。")
    ...
    with c1:
        card_html(
            "正文可配套表述",
            "三个案例分别对应初中物理中的浮力、光学和电学内容，具有较强的代表性和现实意义。通过将深海探测、激光雷达和新能源汽车等现代技术引入课堂，能够帮助学生更直观地理解物理知识在实际生活和科技发展中的作用。",
        )
    with c2:
        card_html(
            "展示顺序建议",
            "先展示关系图，再逐一进入三个案例页面，可以形成“总—分—总”的展示逻辑，使论文结构更清楚、论证更完整。",
        )
...
# =========================
    ...
    header_block("研究数据分析模块", "支持上传课堂前后测、问卷或访谈整理后的 CSV 数据，并绘制统计图。")
    ...
    if df is not None:
        ...
        with st.expander("论文中可以这样写"):
            st.write(
                "为呈现案例教学的实施效果，本文借助 Python 对学生前后测数据进行了统计处理与图形化展示。结果显示，前沿科技案例引入课堂后，学生在学习兴趣和理解程度方面均有提升。"
            )
...
# =========================
    ...
    header_block("图片资料库与素材管理", "用于整理深海探测、激光雷达和新能源汽车相关图片资源。")
    ...
    for tab, key, candidates, title, desc in configs:
        with tab:
            ...
            with col2:
                card_html(
                    "素材说明",
                    f"当前页面用于展示 {title}。建议图片风格尽量统一，便于论文正文和答辩 PPT 的版式保持一致。若没有本地图片，将显示占位图。",
                )
...
# =========================
    ...
    header_block("附录说明与运行指南", "用于论文写作、课堂展示和答辩说明。")
...
project/
├─ app.py
├─ submarine_structure.png   # 可选：直接放同目录
├─ lidar_scan.jpg            # 可选：直接放同目录
├─ ev_platform.jpg           # 可选：直接放同目录
├─ assets/                   # 或放进 assets 文件夹
│  ├─ submarine_structure.png
│  ├─ lidar_scan.jpg
│  └─ ev_platform.jpg
└─ figures/
    ...
    with c1:
        card_html(
            "适合写进论文的方法说明",
            "本文借助 Python 与 Streamlit 构建了可视化展示平台，用于呈现深海探测、激光雷达和新能源汽车三个前沿案例与初中物理知识之间的对应关系。平台集成了原理图展示、案例说明、图片管理和数据分析等功能，有助于增强研究结果的直观性。",
        )
    with c2:
        card_html(
            "答辩使用建议",
            "答辩时可先展示首页总览，再依次进入三个案例页面，最后展示综合关系图和数据分析页，以形成较为清晰的展示顺序。",
        )

    st.markdown(
        """
        <div class="footer-box">
        <b>说明：</b> 当前程序为单文件版本，便于作为论文配套材料提交。后续可根据需要拆分为 pages、components、data 等模块化结构。
        </div>
        """,
        unsafe_allow_html=True,
    )